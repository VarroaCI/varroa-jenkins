...
func TestAction_Power_Hibernated(t *testing.T) {
	...
	srv := testServer(t, func(w http.ResponseWriter, r *http.Request) {
		...
		if !strings.Contains(r.URL.Path, "/clusters/core/controllers/team-a/ctrl-1") {
			t.Errorf("expected path with /clusters/core/..., got %s", r.URL.Path)
		}
		var body map[string]any
		json.NewDecoder(r.Body).Decode(&body)
		spec, ok := body["spec"].(map[string]any)
		if !ok || spec["powerState"] != "Hibernated" {
			t.Errorf("expected spec.powerState=Hibernated, got %v", spec)
		}
		w.Header().Set("Content-Type", "application/json")
...
func TestAction_Power_InvalidState(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "running") || !strings.Contains(err.Error(), "stopped") || !strings.Contains(err.Error(), "hibernated") {
