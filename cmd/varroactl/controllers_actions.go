	...
	registerRootCommand(func(root *cobra.Command) {
		for _, c := range root.Commands() {
			...
			case "power":
				...
				addClusterFlag(ctrlCmd)
				addForceFlag(ctrlCmd)
				c.AddCommand(ctrlCmd)
			case "hibernate":
				ctrlCmd := newControllerSub(newHibernateCmd, runHibernate)
				addClusterFlag(ctrlCmd)
				c.AddCommand(ctrlCmd)
			case "wake":
				ctrlCmd := newControllerSub(newWakeCmd, runWake)
				addClusterFlag(ctrlCmd)
				c.AddCommand(ctrlCmd)
			}
		}
...
func newHibernateCmd() *cobra.Command {
	return &cobra.Command{Short: "Hibernate a controller"}
}
...
func runHibernate(cmd *cobra.Command, args []string) error {
	...
	resp, err := c.HibernateController(cmd.Context(),
		cluster,
		ns,
		name,
	)
	...
	defer func() { _ = resp.Body.Close() }()
	return handleActionResponse(resp, fmt.Sprintf("controller %q hibernating", ns+"/"+name), o)
}
...
func newWakeCmd() *cobra.Command {
	return &cobra.Command{Short: "Wake a hibernated controller"}
}
...
func runWake(cmd *cobra.Command, args []string) error {
	...
	resp, err := c.WakeController(cmd.Context(),
		cluster,
		ns,
		name,
	)
	...
	defer func() { _ = resp.Body.Close() }()
	return handleActionResponse(resp, fmt.Sprintf("controller %q waking", ns+"/"+name), o)
}
...
// Hibernated is deliberately absent: hibernation is not a power state and is
// driven by the hibernate/wake commands.
...
Accepts one of: running, stopped.

Hibernation is not a power state — use "varroactl hibernate controller" to
park a controller and "varroactl wake controller" to wake it.
...
func runPower(cmd *cobra.Command, args []string) error {
	if len(args) < 2 {
		return usagef("usage: power controller NS/NAME {running,stopped}")
	}
	state := strings.ToLower(args[1])
	if state == "hibernated" {
		return usagef("hibernated is not a power state: use \"hibernate controller\" to hibernate a controller and \"wake controller\" to wake it")
	}
