...
// TestController_Describe_Hibernation pins that a parked controller is legible
// in describe: power state stays the spec intent (Running) while the
// hibernation status is reported alongside it.
func TestController_Describe_Hibernation(t *testing.T) {
	testSetup(t)
	srv := testServer(t, func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(map[string]any{
			"namespace": "team-a", "name": "ctrl-1", "phase": "Hibernated",
			"powerState": "Running", "hibernated": true, "hibernatedAt": "2026-07-20T12:30:00Z",
			"version": "2.0", "miteConnected": true, "endpoint": "https://ctrl-1.example.com",
			"routingMode": "subdomain",
		})
	})
	...
	if !strings.Contains(out, "Power State:\tRunning") {
		t.Errorf("describe must show powerState Running (spec intent), got:\n%s", out)
	}
	if !strings.Contains(out, "Hibernated:\ttrue") {
		t.Errorf("describe must show hibernation active, got:\n%s", out)
	}
	if !strings.Contains(out, "Hibernated At:\t2026-07-20T12:30:00Z") {
		t.Errorf("describe must show the hibernation timestamp, got:\n%s", out)
	}
}
