...
import type { PodOverrides, ResourceOverlay, ProbesSpec } from "../types";
...
beforeEach(() => { openConfiguration = false; });
...
function setControllerData(
  overrides: Partial<
    typeof mockControllerData & { podOverrides?: PodOverrides; resourceOverlay?: ResourceOverlay; probes?: ProbesSpec }
  >,
) {
...
describe("ControllerDetail — Tab navigation", () => {
  ...
  it("renders four workspace tabs and lazy-mounts only Overview", () => {
    renderWithProviders(<ControllerDetail />);
...
describe("ControllerDetail — SSE logs lifecycle", () => {
  ...
  it("surfaces the stream error reason instead of looping silently on reconnecting/closed", async () => {
    const user = userEvent.setup();
