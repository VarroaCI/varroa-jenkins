...
export default function ControllerWizard() {
  ...
  const derivedHost = useMemo(() => {
    ...
    // Path mode routes through the dashboard's own host — the BFF rejects any
    // other value — so the subdomain derivation never applies to it.
    if (draft.ingressMode === "path") return draft.config?.dashboardHost ?? "";
    const root = draft.config?.rootDomain;
    if (draft.name && root) return `${draft.name}.${root}`;
    return "";
  }, [draft.name, draft.ingressHost, draft.ingressMode, draft.config]);
  ...
  const buildSubmitBody = (): Record<string, unknown> => {
    ...
    else if (draft.ingressMode === "path" && draft.config?.dashboardHost) ingressSpec.host = draft.config.dashboardHost;
  ...
  const handleDeploy = async () => {
    ...
    try {
      ...
      // Prefer the BFF's own error string ({"error": "..."}), which names the
      // actual rule that rejected the draft, over the generic status copy.
      const serverError =
        e instanceof ApiError && e.body && typeof e.body === "object" && "error" in e.body && typeof e.body.error === "string"
          ? e.body.error
          : "";
      setDeployError(serverError || (e instanceof Error ? e.message : "Deploy failed"));
    } finally {
      setDeploying(false);
    }
  };
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={`${styles.wizard} ${step === 5 ? styles.wizardWithRail : styles.wizardSolo}`}>
              ...
              <div className={styles.field}>
                ...
                {draft.ingressMode === "path" ? (
                  <div className={styles.hint}>
                    {draft.config?.dashboardHost ? (
                      <>
                        Path mode routes through the dashboard host{" "}
                        <span className={styles.hash}>{draft.config.dashboardHost}</span> (leave blank
                        to use it).
                      </>
                    ) : (
                      <>
                        Path mode requires the shared brood dashboard host — check with your cluster
                        admin if you&apos;re unsure.
                      </>
                    )}{" "}
                    The path prefix{" "}
        ...
        {/* Right rail — rendered only when it has content; an empty grid child
