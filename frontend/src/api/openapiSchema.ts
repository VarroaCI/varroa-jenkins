...
function resolveSchemaRefs(
  ...
  if (ref && typeof ref === "string" && ref.startsWith("#")) {
    ...
    if (name && schemas[name] && !visited.has(name)) {
      ...
      const out = resolveSchemaRefs(resolved, schemas, visited);
      // Release the name so a later sibling referencing the same schema is
      // inlined too instead of coming back as a dangling $ref (e.g. ProbeSpec
      // under startup/readiness/liveness). Only a true cycle is kept raw.
      visited.delete(name);
      return out;
    }
