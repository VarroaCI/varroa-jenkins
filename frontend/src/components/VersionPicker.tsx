...
export interface VersionPickerProps {
  ...
  /** Present when the controller this picker edits is currently held by
   *  upgradePolicy:manual (versionStatus.rollReason === "UpgradePending");
   *  rendered as a note, not a behavior change — the picker stays interactive. */
  upgradePendingNote?: string;
}
...
export default function VersionPicker({ versions, value, onChange, disabled, upgradePendingNote }: VersionPickerProps) {
  if (!versions || versions.length === 0) {
    return (
      <>
        {upgradePendingNote && <p className={styles.pendingNote}>{upgradePendingNote}</p>}
        <input
