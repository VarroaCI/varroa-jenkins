...
const MCP_EMITTED_TYPES = [
  "reconcile.triggered", "restart.approved", "catalogsource.synced",
  "controller.created", "controller.updated", "controller.deleted",
  "catalogsource.created", "catalogsource.updated", "catalogsource.deleted",
  "composedbundle.created", "composedbundle.updated", "composedbundle.deleted",
  "jenkinsrole.created", "jenkinsrole.updated", "jenkinsrole.deleted",
  "jenkinsrolebinding.created", "jenkinsrolebinding.updated", "jenkinsrolebinding.deleted",
  "varroarole.created", "varroarole.updated", "varroarole.deleted",
  "varroarolebinding.created", "varroarolebinding.updated", "varroarolebinding.deleted",
  "user.created", "user.updated", "user.deleted",
  "group.created", "group.deleted",
  "versionprofile.created", "versionprofile.updated", "versionprofile.deleted",
  "provisioningdefaults.updated",
];

describe("activity timeline maps", () => {
  it("labels every MCP-emitted activity type", () => {
    const missing = MCP_EMITTED_TYPES.filter((t) => !(t in ACTIVITY_TYPE_MAP));
    expect(missing).toEqual([]);
  });

  it("renders the mcp source with a colour and label", () => {
    expect(SOURCE_COLORS.mcp).toBeDefined();
    expect(SOURCE_LABELS.mcp).toBeDefined();
  });
});
