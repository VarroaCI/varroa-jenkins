...
public class VarroaSecurityRealm extends SecurityRealm {
                ...
                try {
                    if (req instanceof HttpServletRequest && rsp instanceof HttpServletResponse) {
                        ...
                        if (!hasCookie) {
                            ...
                            if (isBrowser) {
                                String loginUrl = System.getenv("VARROA_LOGIN_URL");
                                if (loginUrl != null && !loginUrl.isEmpty()) {
                                    // Build the state URL from the Host header so Varroa can
                                    // redirect the user back to this controller after login.
                                    String rootUrl = jenkins.model.Jenkins.get().getRootUrl();
                                    String state;
                                    if (rootUrl != null) {
                                        state = rootUrl + httpReq.getRequestURI();
                                    } else {
                                        String host = httpReq.getHeader("Host");
                                        String scheme = httpReq.getHeader("X-Forwarded-Proto");
                                        if (scheme == null) scheme = "https";
                                        state = (host != null)
                                            ? scheme + "://" + host + httpReq.getRequestURI()
    ...
    public String getLoginUrl() {
        String url = System.getenv("VARROA_LOGIN_URL");
        return url != null ? url : "/";
    }
    ...
    public HttpResponse doCommenceLogin(StaplerRequest req, @QueryParameter String from) {
        String loginUrl = System.getenv("VARROA_LOGIN_URL");
        if (loginUrl == null) {
            return new HttpRedirect(req.getContextPath() + "/");
        }
        ...
        String state = from != null ? from : "/";
        String redirectUrl = loginUrl + "?state=" + URLEncoder.encode(state, StandardCharsets.UTF_8);
