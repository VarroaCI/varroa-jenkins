...
grep -ci "oauth2-proxy" <<< "$HIVE_RENDER" > /dev/null 2>&1 && check "No oauth2-proxy resources" 1 || check "No oauth2-proxy resources" 0
grep -c "grafana" <<< "$HIVE_RENDER" > /dev/null 2>&1 && check "No grafana resources" 1 || check "No grafana resources" 0
grep -c "prometheus" <<< "$HIVE_RENDER" > /dev/null 2>&1 && check "No prometheus resources" 1 || check "No prometheus resources" 0
