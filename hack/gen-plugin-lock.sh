...
#                      and the operator's embedded version-profile seed content
#
...
# profile.yaml and plugins.yaml pair is emitted under
# internal/controller/pluginlock/seed/, embedded into the operator binary and
# seeded into the cluster at runtime by VersionProfileSeedReconciler.
#
...
#
# Core seed plugins are the plugins Varroa pins:
#   configuration-as-code, role-strategy, instance-identity, kubernetes,
#   workflow-aggregator, workflow-cps-global-lib, mcp-server, job-dsl,
#   docker-workflow, github-branch-source, git, nodejs, jdk-tool,
#   maven-plugin, config-file-provider
...
SEED_DIR="${REPO_ROOT}/internal/controller/pluginlock/seed"

CORE_PLUGINS="configuration-as-code role-strategy instance-identity kubernetes workflow-aggregator workflow-cps-global-lib mcp-server job-dsl docker-workflow github-branch-source git nodejs jdk-tool maven-plugin config-file-provider"
...
# emit_seed_files writes the profile.yaml and plugins.yaml pair embedded by
# internal/controller/pluginlock/seed.go for a given version. Arguments:
#   1 — full version string (e.g. "2.552") — used for spec.version and the seed
#       directory name
...
emit_seed_files() {
    ...
    # Write plugins.yaml — raw content only, embedded verbatim into the
    # pluginset ConfigMap's data["plugins.yaml"] key at seed time.
    cat > "${dir}/plugins.yaml" <<PLUGEOF
${header}
core:
...
PLUGEOF
    ...
    # Write profile.yaml — JenkinsVersionProfileSpec fields only. pluginSetRef
    # is not stored here: it is always "<profile-name>-pluginset", derived by
    # the seed reconciler rather than duplicated in the source file.
    cat > "${dir}/profile.yaml" <<PROFEOF
${header}
version: "${ver}"
channel: ${channel}
...
PROFEOF
    ...
    echo "  Wrote ${dir}/profile.yaml" >&2
    echo "  Wrote ${dir}/plugins.yaml" >&2
}

# Wipe the stale seed tree so versions removed from the manifest leave no
# embedded artifacts. The seed layout nests one directory per version (unlike
# the flat file list this replaced), so the clear must be recursive.
rm -rf "${SEED_DIR}"
mkdir -p "${SEED_DIR}"
...
while IFS='|' read -r version channel recommended eol resolveVersion; do
    ...
    # Emit the operator's embedded seed files for this version.
    emit_seed_files "$version" "$OUTPUT" "$channel" "$recommended" "$eol" "$resolveVersion"
