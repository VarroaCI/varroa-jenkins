...
func TestFindUnresolvedVariables(t *testing.T) {
	tests := []struct {
		...
		noJcasc  bool // scan as non-casc content: secret-source refs count as unresolved
		want     []string
	}{
		{
			name:    "secret source counts as unresolved outside casc-applied content",
			content: "${readFile:/run/secrets/ssh-privatekey}",
			noJcasc: true,
			want:    []string{"readFile:/run/secrets/ssh-privatekey"},
		},
		{
			name:     "empty content",
			content:  "",
		...
		{
			name:    "JCasC readFile secret source skipped",
			content: "${readFile:/run/secrets/ssh-privatekey} ${missing}",
			want:    []string{"missing"},
		},
		{
			name:    "JCasC base64 secret source skipped",
			content: "${base64:c2VjcmV0}",
			want:    []string{},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := FindUnresolvedVariables(tt.content, tt.userVars, !tt.noJcasc)
