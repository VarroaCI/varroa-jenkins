...
func TestTicket_MintVerifyRoundTrip(t *testing.T) {
	...
	caller := &Claims{Subject: "alice", Email: "a@x", Name: "Alice", PreferredUsername: "alice", Groups: []string{"devs"}}
	...
	if got.PreferredUsername != "alice" {
		t.Errorf("PreferredUsername = %q, want %q (ticket must carry it for kind:User bindings)", got.PreferredUsername, "alice")
	}
}
