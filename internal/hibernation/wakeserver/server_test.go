...
func wakeController(name, namespace string, phase v1alpha1.ControllerPhase, powerState string, hibernated bool) *v1alpha1.Controller {
...
func TestServerMapsSubdomainAndPathRequests(t *testing.T) {
	subdomain := wakeController("ci", "team-a", v1alpha1.ControllerPhaseHibernated, "", true)
	pathMode := wakeController("shared", "team-b", v1alpha1.ControllerPhaseRunning, "Running", false)
	...
	t.Run("explicit host ResolveHost parity", func(t *testing.T) {
		explicit := wakeController("explicit", "team-c", v1alpha1.ControllerPhaseRunning, "Running", false)
	...
	t.Run("subdomain host falls back after path lookup miss", func(t *testing.T) {
		decoy := wakeController("y", "x", v1alpha1.ControllerPhaseHibernated, "", true)
		waker := &stubWaker{}
...
func TestServerRejectsEmptyOrUnresolvedHosts(t *testing.T) {
	pathMode := wakeController("shared", "team-b", v1alpha1.ControllerPhaseHibernated, "", true)
	pathMode.Spec.IngressSpec = &v1alpha1.IngressSpec{Mode: v1alpha1.RoutingModePath}
...
func TestServerWakeDispatch(t *testing.T) {
	t.Run("hibernated CAS exactly once", func(t *testing.T) {
		cr := wakeController("ci", "team-a", v1alpha1.ControllerPhaseHibernated, "", true)
		waker := &stubWaker{}
	...
	t.Run("stopped never wakes and always returns JSON", func(t *testing.T) {
		cr := wakeController("ci", "team-a", v1alpha1.ControllerPhaseStopped, "Stopped", false)
		waker := &stubWaker{}
		srv := &Server{Lister: stubLister{[]*v1alpha1.Controller{cr}}, Waker: waker, RootDomain: func() string { return "example.com" }}
	...
	t.Run("connected nudges owning shard", func(t *testing.T) {
		cr := wakeController("ci", "team-a", v1alpha1.ControllerPhaseConnected, "Running", false)
		waker := &stubWaker{}
		srv := &Server{Lister: stubLister{[]*v1alpha1.Controller{cr}}, Waker: waker, RootDomain: func() string { return "example.com" }}
...
func TestServerContentNegotiation(t *testing.T) {
	cr := wakeController("ci", "team-a", v1alpha1.ControllerPhaseProvisioning, "Running", false)
...
func TestServerStatusContract(t *testing.T) {
	cr := wakeController("ci", "team-a", v1alpha1.ControllerPhaseRunning, "Running", false)
...
func TestServerLogsWakeNudgeAndUnknownController(t *testing.T) {
	...
	hibernated := wakeController("sleeping", "team-a", v1alpha1.ControllerPhaseHibernated, "", true)
	connected := wakeController("ready", "team-b", v1alpha1.ControllerPhaseConnected, "Running", false)
	waker := &stubWaker{}
	srv := &Server{
