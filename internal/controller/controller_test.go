...
type testClient struct {
	...
	getSecretCalls       map[string]int               // name -> GetSecret call count

	// hibernation CAS tracking (SetHibernated).
	hibernatedWrites []struct {
		name, namespace string
		want            bool
	}
	hibernatedChanged bool
	hibernatedErr     error
...
func (t *testClient) GetSecret(_ context.Context, name, namespace string) (map[string][]byte, error) {
	if t.getSecretCalls == nil {
		t.getSecretCalls = map[string]int{}
	}
	t.getSecretCalls[name]++
...
func (t *testClient) SetHibernated(_ context.Context, name, namespace string, want bool) (bool, error) {
	...
	if t.hibernatedErr != nil {
		return false, t.hibernatedErr
	}
	return t.hibernatedChanged, nil
}
...
func TestPowerOffClearsMiteConnection(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"Stopped", "Stopped", false, v1alpha1.ControllerPhaseStopped},
		{"Hibernated", "", true, v1alpha1.ControllerPhaseHibernated},
	} {
...
func TestResolveBundleForController_JCascSecretSourceAllowed(t *testing.T) {
	// ${readFile:...} is a JCasC secret source resolved by Jenkins' own
	// configuration-as-code interpolation at Jenkins startup, not by Varroa,
	// and must reach Jenkins byte-for-byte (unlike ^${var}, which JCasC
	// treats as inert literal text).
	client := newTestClientWithBundle()
	client.composedBundles["test-bundle"] = &v1alpha1.ComposedBundle{
		Status: v1alpha1.ComposedBundleStatus{
			Phase:      v1alpha1.ComposedBundleReady,
	...
	mat, _, _, err := rec.resolveBundleForController(context.Background(), cr)
	if err != nil {
		t.Fatalf("expected no completeness error for ${readFile:...} secret source, got: %v", err)
	}
	if !strings.Contains(mat.JenkinsYAML, "${readFile:/run/secrets/ssh-privatekey}") {
		t.Errorf("expected readFile secret source to be preserved verbatim, got: %q", mat.JenkinsYAML)
	}
}
...
func TestFindUnresolvedVars_None(t *testing.T) {
	vars := findUnresolvedVars(true, "jenkins:\n  key: value\n", "", "", "")
	if len(vars) != 0 {
...
func (c *captureTransport) StreamDegraded(ns, name string) (string, bool) {
	return c.streamDegradedReason, c.streamDegraded
}
...
func TestReconcileController_HibernatedRefreshesImageStaleness(t *testing.T) {
	...
	cr := &v1alpha1.Controller{
		ObjectMeta: metav1.ObjectMeta{Name: "test", Namespace: "ns", UID: testUID, CreationTimestamp: recent},
		Spec:       v1alpha1.ControllerSpec{MiteSpec: &v1alpha1.MiteSpec{Image: "mite:desired"}},
		Status:     v1alpha1.ControllerStatus{Phase: v1alpha1.ControllerPhaseConnected, Hibernated: true},
	}
