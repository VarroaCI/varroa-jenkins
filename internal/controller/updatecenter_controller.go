...
func (r *UpdateCenterReconciler) reconcileSeedImport(ctx context.Context, uc *v1alpha1.UpdateCenter, status *v1alpha1.UpdateCenterStatus, logger *slog.Logger) {
	...
	for _, ref := range refs {
		...
		digest, err := r.resolveSeedRef(ctx, ref)
		if err != nil {
		...
		// Track this digest in the new set regardless of skip.
		...
		if err := r.importSeedRef(ctx, ref, token); err != nil {
...
func (r *UpdateCenterReconciler) resolveSeedRef(ctx context.Context, ref string) (string, error) {
	store, err := r.newRegistryStore(ref, oci.RegistryOptions{})
	if err != nil {
...
func (r *UpdateCenterReconciler) importSeedRef(ctx context.Context, ref, token string) error {
	// 1. Create a RegistryStore for the source ref.
	srcStore, err := r.newRegistryStore(ref, oci.RegistryOptions{})
	if err != nil {
