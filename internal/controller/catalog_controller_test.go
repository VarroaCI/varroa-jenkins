...
func TestReconcileComposedBundle_ItemRefDisappearanceReMaterializes(t *testing.T) {
	...
	cb := &v1alpha1.ComposedBundle{
		ObjectMeta: metav1.ObjectMeta{Name: "item-gone-cb", Namespace: "ns"},
		Spec: v1alpha1.ComposedBundleSpec{
			Inputs: []v1alpha1.ComposedInput{{ItemRef: &v1alpha1.ComposedItemRef{Name: "jcasc-1"}}},
		},
	}
	...
	rec := newComposedBundleReconciler(tc, t)
	rec.ReconcileComposedBundle(ctx, cb)
	if got := tc.lastStatus(); got == nil || got.Phase != v1alpha1.ComposedBundleReady {
		t.Fatalf("first reconcile phase = %v, want Ready", tc.lastStatus())
	}
	cb.Status = *tc.lastStatus()

	// The catalog drops the item — a renamed path, or a prune after it left
	// the index. The ComposedBundle spec is untouched.
	if err := crdstore.Delete[v1alpha1.CatalogItem](ctx, tc.store, "jcasc-1", "ns"); err != nil {
		t.Fatalf("delete catalog item: %v", err)
	}

	rec.ReconcileComposedBundle(ctx, cb)
	st := tc.lastStatus()
	if st.Phase != v1alpha1.ComposedBundleInvalid {
		t.Errorf("phase = %q, want Invalid once the only input stopped resolving", st.Phase)
	}
	if got := st.ObservedRevisions["0"]; got != itemMissingRevision {
		t.Errorf("observedRevisions[0] = %q, want %q", got, itemMissingRevision)
	}
	// Last-good content is retained rather than blanked.
	if st.ContentRef != "item-gone-cb-content" {
		t.Errorf("contentRef = %q, want the previous content retained", st.ContentRef)
	}

	// The sentinel must also settle: a still-missing item is not fresh drift,
	// so the next tick has to take the skip path instead of recomposing on
	// every pass forever.
	cb.Status = *st
	gvr, err := crdstore.GVRFor[v1alpha1.ComposedBundle]()
	if err != nil {
	...
	before := len(tc.store.StatusPatches(gvr))
	rec.ReconcileComposedBundle(ctx, cb)
	if after := len(tc.store.StatusPatches(gvr)); after != before {
		t.Errorf("a persistently missing item recomposed again (%d new status patches); the sentinel should settle", after-before)
	}
}
...
// TestReconcileComposedBundle_TransientItemReadDoesNotFabricateDeletion: the
// missing-item sentinel must fire on a real deletion only. crdstore.Get also
// errors on timeouts and RBAC failures, and treating those as a deletion would
// flip a healthy bundle to Invalid on an API blip — then suppress drift for the
// rest of the outage, because the sentinel matches itself.
func TestReconcileComposedBundle_TransientItemReadDoesNotFabricateDeletion(t *testing.T) {
	ctx := context.Background()
	tc := newCatalogTestClient()
	item := jcascItem("jcasc-1", "jenkins:\n  systemMessage: \"v1\"\n")
	item.Status.ContentHash = "hash-v1"
	...
	cb := &v1alpha1.ComposedBundle{
		ObjectMeta: metav1.ObjectMeta{Name: "flaky-read-cb", Namespace: "ns"},
		Spec: v1alpha1.ComposedBundleSpec{
			Inputs: []v1alpha1.ComposedInput{{ItemRef: &v1alpha1.ComposedItemRef{Name: "jcasc-1"}}},
		},
	}
	...
	itemGVR, err := crdstore.GVRFor[v1alpha1.CatalogItem]()
	if err != nil {
		t.Fatal(err)
	}
	tc.store.FailAlways("get", itemGVR, errors.New("etcdserver: request timed out"))

	cbGVR, err := crdstore.GVRFor[v1alpha1.ComposedBundle]()
	if err != nil {
		t.Fatal(err)
	}
	before := len(tc.store.StatusPatches(cbGVR))
	rec.ReconcileComposedBundle(ctx, cb)

	if after := len(tc.store.StatusPatches(cbGVR)); after != before {
		t.Errorf("a transient read error recomposed the bundle (%d new status patches); it is not a deletion", after-before)
	}
}
