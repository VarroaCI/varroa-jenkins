...
func (c *ConfigCRUD) composeBundle(data []byte, _ /* validateOnly — the BFF distinguishes by endpoint, operator side always composes */ bool) []byte {
	...
	// Concatenate all YAML sections for variable resolution.
	var allYAML string
	for _, s := range []string{preview.BundleYAML, preview.JenkinsYAML, preview.PluginsYAML, preview.ItemsYAML, preview.RbacYAML} {
		if s != "" {
			allYAML += s + "\n"
		}
	}
	preview.UnresolvedVariables = bundle.FindUnresolvedVariables(allYAML, spec.Variables)
