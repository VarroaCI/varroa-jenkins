...
const (
	...
	// itemMissingRevision is the observedRevisions value recorded for an
	// itemRef input whose CatalogItem does not resolve. It is deliberately not
	// valid sha256 hex, so it can never collide with a real content hash.
	itemMissingRevision = "<missing>"

	// ucFullPassInterval bounds how long the update-center arm may rely on its
	// digest before deriving every item again. The digest covers the inventory
	// and the version profiles, so it catches every upstream change; this pass
	// exists to repair what the digest cannot see — items deleted or edited
	// out of band, and writes that failed with a warning on an earlier pass.
	ucFullPassInterval = 30 * time.Minute
...
type CatalogReconciler struct {
	...
	// ucLastFullPass is when the update-center arm last wrote every derived
	// item. It gates the periodic repair pass described on ucFullPassInterval.
	// Process-local by design: it is a cost optimization, never a correctness
	// input, so losing it on restart or leader change simply forces one full
	// pass. Only the leader-elected catalog ticker touches it, single-threaded.
	ucLastFullPass time.Time

	// ucLastFullPassMessage is the status message that pass produced, replayed
	// by markUCUnchanged. A skipped pass must not invent a message, and it
	// cannot read the previous one off src: markSyncing has already
	// overwritten the phase by then, so an error left by setError would
	// otherwise be reported alongside Ready until the next derive.
	ucLastFullPassMessage string
}
...
func (r *CatalogReconciler) Reconcile(ctx context.Context, src *v1alpha1.CatalogSource) {
	...
	for _, entry := range index.Items {
		...
		itemWarnings, _ := r.writeCatalogItem(ctx, src, item, desired)
		warnings = append(warnings, itemWarnings...)
	}

	// GC stale CatalogItems that were present on the cluster but are no longer
	// in the index.
	_ = r.pruneCatalogItems(ctx, src, desired)
	...
	for i, input := range cb.Spec.Inputs {
		...
		if ref := input.ItemRef; ref != nil {
			...
			if ref.Namespace != "" {
				...
				// Fall back only when the local namespace genuinely has no
				// such item. Falling back on a transient local error would
				// let the operator-namespace NotFound overwrite it, and the
				// caller cannot then tell "absent everywhere" from "we could
				// not read it here".
				if (apierrors.IsNotFound(err) || (err == nil && item == nil)) &&
					r.operatorNamespace != "" && r.operatorNamespace != cb.Namespace {
					item, err = crdstore.Get[v1alpha1.CatalogItem](ctx, r.store, ref.Name, r.operatorNamespace)
				}
			}
			// An item that no longer resolves is recorded with a sentinel
			// rather than skipped. Skipping leaves no observedRevisions entry,
			// so the gate below sees no drift and the bundle goes on serving
			// content for an input that has since disappeared. The sentinel
			// makes the disappearance itself a drift event exactly once — the
			// composer then reports it via result.Missing — while the recorded
			// value stops it from forcing a recompose on every later tick.
			hash := itemMissingRevision
			switch {
			case err == nil && item != nil:
				hash = item.Status.ContentHash
			case err != nil && !apierrors.IsNotFound(err):
				// A transient read failure is not a deletion. Recording the
				// sentinel here would flip a healthy bundle to Invalid on an
				// API blip, and then suppress drift for the rest of the outage
				// because the sentinel matches itself. Same policy as the git
				// and OCI branches below: record nothing this pass.
				logger.Warn("itemRef read failed; leaving the recorded revision alone",
					"input", i, "item", ref.Name, "error", err)
				continue
			}
...
func (r *CatalogReconciler) isSyncDue(src *v1alpha1.CatalogSource) bool {
	...
	return manualSyncRequested(src)
}

// manualSyncRequested reports whether the sync-requested-at annotation asks for
// a pass newer than the last completed one.
//
// It is also what exempts an operator-requested sync from the update-center
// skip gate. Someone pressing sync is usually trying to repair drift the
// digest cannot see, so answering with a no-op would defeat the button.
func manualSyncRequested(src *v1alpha1.CatalogSource) bool {
	annoVal := src.Annotations[syncRequestedAtAnno]
	if annoVal == "" {
		return false
	}
	if src.Status.LastSyncTime == nil {
	...
	return err == nil && requestedAt.After(src.Status.LastSyncTime.Time)
}
