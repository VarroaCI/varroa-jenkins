...
func checkIngressHost(ctx context.Context, deps Deps, draft *v1alpha1.Controller, defaults *v1alpha1.ProvisioningDefaults) Check {
	// Path mode shares the dashboard host by design — the availability check
	// would flag it "already claimed" on every path-mode create.
	if draft.Spec.IngressSpec != nil && draft.Spec.IngressSpec.RoutingMode() == v1alpha1.RoutingModePath {
		return Check{ID: "ingress-host", Status: "pass", Message: "path mode shares the dashboard host; no availability check"}
	}
	host := ""
