...
type Brood interface {
	...
	// Update applies a merge-patch to a controller on the target cluster via SSA.
	// fieldManager identifies the owning client; force=true re-asserts ownership over conflicting fields.
	Update(ctx context.Context, cluster, ns, name string, patch json.RawMessage, fieldManager string, force bool) (*v1alpha1.Controller, []bus.Check, error)
...
func (f *busBrood) Update(ctx context.Context, cluster, ns, name string, patch json.RawMessage, fieldManager string, force bool) (*v1alpha1.Controller, []bus.Check, error) {
	...
	if err := json.Unmarshal(resp, &updateResp); err != nil {
		return nil, nil, fmt.Errorf("update decode: %w", err)
	}
	if updateResp.Code != "" {
		return nil, updateResp.Checks, &BroodError{Code: updateResp.Code, Msg: updateResp.Error, Conflicts: updateResp.Conflicts}
	}
	if updateResp.Error != "" {
		return nil, nil, fmt.Errorf("update: %s", updateResp.Error)
	}
	var cr v1alpha1.Controller
	if err := json.Unmarshal(updateResp.Item, &cr); err != nil {
		return nil, nil, fmt.Errorf("update unmarshal: %w", err)
	}
	return &cr, nil, nil
}
