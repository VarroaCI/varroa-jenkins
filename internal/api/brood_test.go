...
func (f *fakeBroodClient) ApplyControllerCRD(_ context.Context, _ *v1alpha1.Controller) error {
	return nil
}
func (f *fakeBroodClient) ApplyControllerSpecSSA(_ context.Context, _, _ string, _ map[string]any, _ string, _ bool) (*v1alpha1.Controller, error) {
	return nil, nil
}
func (f *fakeBroodClient) TransitionPowerState(_ context.Context, _, _, _, _ string) (bool, error) {
	return false, nil
}
...
func TestBroodErrorConflicts(t *testing.T) {
	...
	_, _, err := bf.Update(context.Background(), "dev-cluster", "team-a", "test-ctrl", json.RawMessage(`{"spec":{"version":"2.0"}}`), "varroa-ui", false)
	if err == nil {
		t.Fatal("expected error")
	}
	var fe *BroodError
