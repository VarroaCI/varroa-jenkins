...
type fakeResourceClient struct {
	...
	// controllers backs Get/Apply/ListControllerCRDs when non-nil; the
	// zero value preserves the legacy nil,nil stubs.
...
func (f *fakeResourceClient) ApplyControllerCRD(_ context.Context, cr *v1alpha1.Controller) error {
	if f.controllers != nil {
		f.controllers[cr.Name] = cr.DeepCopy()
	}
	return nil
}
func (f *fakeResourceClient) ApplyControllerSpecSSA(_ context.Context, namespace, name string, spec map[string]any, _ string, _ bool) (*v1alpha1.Controller, error) {
	if f.controllers != nil {
		if ctrl, ok := f.controllers[name]; ok {
			return ctrl.DeepCopy(), nil
		}
	}
	return nil, nil
}
func (f *fakeResourceClient) TransitionPowerState(_ context.Context, name, ns, from, to string) (bool, error) {
	return false, nil
}
