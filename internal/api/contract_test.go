...
func (f *fakeBrood) Create(ctx context.Context, cluster string, req ControllersCreateArgs) (*v1alpha1.Controller, []bus.Check, error) {
	...
	if err := f.client.ApplyControllerCRD(ctx, &cr); err != nil {
		return nil, nil, err
	}
	return &cr, nil, nil
}
...
func (f *fakeBrood) Update(ctx context.Context, cluster, ns, name string, patch json.RawMessage, fieldManager string, force bool) (*v1alpha1.Controller, []bus.Check, error) {
	...
	_, err = f.client.ApplyControllerSpecSSA(ctx, ns, name, specPatch, fieldManager, force)
	if err != nil {
		return nil, nil, err
	}
