...
// Backfill is the interface for reading activity history. The handlers use it
// to remain mode-agnostic (JetStream vs in-memory ring). Query is part of the
// interface, not an optional upgrade: a filtered read implemented by truncating
// an unscoped Recent to the limit BEFORE filtering returns incomplete or empty
// pages whenever the newest events belong to other controllers — every
// implementation must filter first.
type Backfill interface {
	...
	// Query returns filtered, paginated activity history.
	Query(ctx context.Context, query Query) (Page, error)
	// Retention reports the retention mode ("on"/"off") and configured days.
...
// newestFirst adapts Store.List (oldest-first ring order) to the Backfill
// contract: jetstreamBackfill reads end-anchored and returns newest-first, so
// the ring must too — otherwise a full ring serves the OLDEST events and
// Recent/Query silently drop the newest audit history.
func (rb *ringBackfill) newestFirst(controller string) []Event {
...
func (rb *ringBackfill) Query(_ context.Context, q Query) (Page, error) {
	return querySlice(rb.newestFirst(""), q, "off", 0)
}
