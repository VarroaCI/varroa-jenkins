...
// Cluster is always stamped by the shared Publisher.
type Event struct {
	Timestamp   time.Time `json:"timestamp"`
	Type        string    `json:"type"`
	Source      string    `json:"source"`          // "mite", "operator", "user", "jenkins"
	Actor       string    `json:"actor,omitempty"` // OIDC email, "operator", or empty
