...
func TestHandleCreateController_LocalFallback_OmitsWakeTokenShape(t *testing.T) {
	role := &v1alpha1.VarroaRole{
	...
	srv := NewServer(&Dependencies{
		...
		Store:             storeFromFake(fc),
		Logger:            slog.Default(),
		Brood:             nil, // forces local-fallback branch
...
func TestHandleUpdateController_LocalFallback_StripsWakeToken(t *testing.T) {
	role := &v1alpha1.VarroaRole{
