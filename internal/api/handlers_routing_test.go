...
func TestUpdateControllerRoutingValidation(t *testing.T) {
	...
	t.Run("mode flip rejected", func(t *testing.T) {
		srv, client := newRoutingTestServer()
		seed(client, "")
		w := patchController(t, srv, `{"spec":{"ingressSpec":{"mode":"path"}}}`)
		if w.Code != http.StatusBadRequest {
			t.Fatalf("expected 400, got %d: %s", w.Code, w.Body.String())
		}
		if !strings.Contains(w.Body.String(), "immutable") {
			t.Errorf("error body should say immutable, got %s", w.Body.String())
		}
	})

	t.Run("removing ingressSpec from path controller rejected", func(t *testing.T) {
		srv, client := newRoutingTestServer()
		client.controllers["ci"] = &v1alpha1.Controller{
			ObjectMeta: metav1.ObjectMeta{Name: "ci", Namespace: "team-a"},
			Spec:       v1alpha1.ControllerSpec{IngressSpec: &v1alpha1.IngressSpec{Host: "varroa.example.com", Mode: v1alpha1.RoutingModePath}},
		...
		crdstore.MustSeed(client.crdStore, client.controllers["ci"])
		w := patchController(t, srv, `{"spec":{"ingressSpec":null}}`)
		if w.Code != http.StatusBadRequest {
			t.Fatalf("expected 400, got %d: %s", w.Code, w.Body.String())
		}
	})
	...
	t.Run("invalid enum on update rejected", func(t *testing.T) {
		srv, client := newRoutingTestServer()
		seed(client, "")
		w := patchController(t, srv, `{"spec":{"ingressSpec":{"mode":"proxy"}}}`)
		if w.Code != http.StatusBadRequest {
			t.Fatalf("expected 400, got %d: %s", w.Code, w.Body.String())
		}
	})

	t.Run("invalid annotation key on update rejected", func(t *testing.T) {
		srv, client := newRoutingTestServer()
		seed(client, "")
		w := patchController(t, srv, `{"spec":{"ingressSpec":{"mode":"subdomain","annotations":{"not a valid key!":"v"}}}}`)
		if w.Code != http.StatusBadRequest {
			t.Fatalf("expected 400, got %d: %s", w.Code, w.Body.String())
		}
		if !strings.Contains(w.Body.String(), "ingressSpec.annotations") {
