...
- Activity (`internal/api/activity/`): `Store` is the in-memory ring buffer used only
  ...
  JetStream stream (`varroa_activity`, subjects `activity.<ns>.<ctrl>` /
  `activity._global`) via an **ephemeral, end-anchored consumer**; backfill is
  pull/paginated (cursor-based `Query`) — live tailing is a separate path.
