...
// OperatorWakeSubject returns the subject for waking a per-controller
// goroutine for immediate reconciliation.
...
type ControllersUpdateRequest struct {
	Namespace    string          `json:"namespace"`
	Name         string          `json:"name"`
	Patch        json.RawMessage `json:"patch"`                  // {"spec": {...}} merge patch; BFF pre-validated
