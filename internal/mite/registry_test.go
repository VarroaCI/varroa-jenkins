...
// TestUnregisterIfEpochProtectsReplacementConnection pins that a superseded
// stream's teardown cannot clear the connection that replaced it. Checking the
// epoch and unregistering as two separate registry calls left a window where a
// reconnect landing between them was torn down anyway.
func TestUnregisterIfEpochProtectsReplacementConnection(t *testing.T) {
	reg := NewRegistry()

	reg.Register("ctl", "varroa", nil, "stream-a", "v1", time.Now().Add(time.Hour))
	oldEpoch, ok := reg.GetEpoch("ctl", "varroa")
	if !ok {
		t.Fatal("first connection was not registered")
	}

	// The mite reconnects on a new stream before the old one tears down.
	reg.Register("ctl", "varroa", nil, "stream-b", "v1", time.Now().Add(time.Hour))
	newEpoch, _ := reg.GetEpoch("ctl", "varroa")
	if oldEpoch == newEpoch {
		t.Fatal("reconnect did not advance the connection epoch")
	}

	if reg.UnregisterIfEpoch("ctl", "varroa", oldEpoch) {
		t.Fatal("a superseded stream's teardown unregistered the live connection")
	}
	if _, stillThere := reg.Get("ctl", "varroa"); !stillThere {
		t.Fatal("live connection was removed by the superseded stream's teardown")
	}

	// The live stream's own teardown still works.
	if !reg.UnregisterIfEpoch("ctl", "varroa", newEpoch) {
		t.Fatal("the live stream's teardown was rejected")
	}
}
