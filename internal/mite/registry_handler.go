...
func (h *RegistryHandler) OnConnect(name, ns, version string, certExpiry interface{}, _ Sender, stream interface{}) {
	expiry, _ := certExpiry.(time.Time)
	h.reg.Register(name, ns, nil, stream, version, expiry)
...
func (h *RegistryHandler) OnDisconnect(name, ns string) {
	h.reg.Unregister(name, ns)
