...
- **`CommandStream`** (`server.go:209`) — first message must be `Hello`;
  ...
  `StreamHandler`; sends go through one `sendMu`-guarded closure.
- **`StreamHandler`** (`stream_handler.go`) — `RegistryHandler` backs the
  ...
  never replays stale commands). Imperative one-shots (safe-restart, webhook
...
- New `CommandStream` message variants need a case in **both**
  ...
  `OnContentResponse`, `OnJenkinsActivity`, `OnPluginInventory`, `OnDisconnect`.
- `BusHandler` per-connection maps (`cancels`, `certExpiry`, `pendingAcks`,
  `replayCmds`, `pendingContent*`) are all keyed `"ns/name"` under one `mu`;
  follow that pattern rather than adding a second mutex.
